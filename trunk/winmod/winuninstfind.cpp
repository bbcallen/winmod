...
void CWinUninstFind::DoReadStandardValues()
{
    ...
    {   // ..\\Uninstall\\..\\[DisplayName]
        HRESULT hr = hRegKeyUninstall.QueryDWORDValue(WINMOD_REGVALUE_UNINST_SYSTEMCOMPONENT, m_dwSysmteComponent);
        if (FAILED(hr))
            m_dwSysmteComponent = 0;            // no return

        hr = hRegKeyUninstall.ExQueryStringValue(WINMOD_REGVALUE_UNINST_DISPLAYNAME, m_strDisplayName);
        if (FAILED(hr))
            m_strDisplayName = L"";             // no return

        hr = hRegKeyUninstall.ExQueryStringValue(WINMOD_REGVALUE_UNINST_PUBLISHER, m_strPublisher);
        if (FAILED(hr))
            m_strPublisher = L"";               // no return

        hr = hRegKeyUninstall.ExQueryStringValue(WINMOD_REGVALUE_UNINST_UNINSTALLSTRING, m_strUninstallString);
        if (FAILED(hr))
